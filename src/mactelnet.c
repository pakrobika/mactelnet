...
int main(int argc, char **argv) {
	...
	while (1) {
		c = getopt(argc, argv, "nqot:u:p:U:vh?Aa:");
	...
	if (argc - optind < 1 || print_help) {
		print_version();
		fprintf(stderr,
				_("Usage: %s <MAC> [-nqoA] [-a <path>] [-t <timeout>] [-u <user>] [-p <password>] [-U <user>]\n"),
				argv[0]);
		...
		if (print_help) {
			fprintf(stderr,
					_("\nParameters:\n"
					  ...
					  "  -t <timeout>   Amount of seconds to wait for a successufl connection.\n"
	...
	if (!parse_mac_address(argv[optind], dstmac)) {
